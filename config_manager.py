...
def show_admin_panel(mode: str = "foundation") -> None:
    ...
    file_type = st.radio("Choose file type:", source_options, horizontal=True, key=f"{mode}_filetype")

    uploaded_file = st.file_uploader("Upload CSV or Excel", type=["csv", "xlsx"], key=f"{mode}_{file_type}")
    ...
    if uploaded_file:
        try:
            ...
            if validate_sample_columns(df):
                process_uploaded_file(uploaded_file, file_type, mode)
                st.success(f"{file_type} Sample Loaded ✅")
            else:
                st.error("❌ Uploaded file has no valid columns.")
        except Exception as e:
